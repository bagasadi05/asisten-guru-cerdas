...
const TasksPDF = React.forwardRef<HTMLDivElement, TasksPDFProps>(({ items, title }, ref) => {
    
    const getStatusColor = (status: string) => {
        switch(status) {
            case 'Selesai': return 'text-green-700';
            case 'Dikerjakan': return 'text-yellow-700';
            case 'Belum': return 'text-red-700';
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-gray-100 border-b-2 border-gray-300">
                            <th className="p-3 text-sm font-bold uppercase text-gray-600 w-2/5">Judul Tugas</th>
                            <th className="p-3 text-sm font-bold uppercase text-gray-600">Tenggat</th>
                            <th className="p-3 text-sm font-bold uppercase text-gray-600">Kategori</th>
                    ...
                    <tbody>
                        {items.map(item => (
                            <tr key={item.id} className="border-b border-gray-200">
                                ...
                                <td className="p-3 align-top">{new Date(item.dueDate).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' })}</td>
                                <td className="p-3 align-top">{item.category}</td>
                                <td className={`p-3 align-top font-bold ${getStatusColor(item.status)}`}>{item.status}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
                {items.length === 0 && <p className="text-center text-gray-500 mt-8">Tidak ada data tugas untuk ditampilkan.</p>}
            </main>
